...
describe('The database', () => {
  it('development should be reachable', async () => {
    const db = await MongoClient.connect(configDev.database.dsn, mongoOptions);
    expect(db).to.not.be.null;
    await db.close();
  });
  it('test should be reachable', async () => {
    const db = await MongoClient.connect(configTest.database.dsn, mongoOptions);
    expect(db).to.not.be.null;
    await db.close();
  });
  it('production should be reachable', async () => {
    const db = await MongoClient.connect(configProd.database.dsn, mongoOptions);
    expect(db).to.not.be.null;
    await db.close();
  });
});
